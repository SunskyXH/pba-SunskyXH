...
void WdWddW_Spring3(
  ...
  const Eigen::Matrix3d m = (Eigen::Matrix3d::Identity() - (u01 * u01.transpose())) / length;
  const Eigen::Matrix3d ddC[2][2] = {{m, -m}, {-m, m}};
  // ddw = k*dC*dC+k*C*ddC
  ddw[0][0] = n + stiffness * C * ddC[0][0];
  ddw[1][1] = n + stiffness * C * ddC[1][1];
  ddw[0][1] = -n + stiffness * C * ddC[0][1];
  ddw[1][0] = -n + stiffness * C * ddC[1][0];
}
