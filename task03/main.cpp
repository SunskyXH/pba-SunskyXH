    ...
    for (unsigned int jx = 0; jx < num_div; ++jx) {
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          unsigned int weight = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx];
          Eigen::Vector2f representative_particle_pos = acc.grid2cg[jy * num_div + jx];
          particles[ip].force += gravitational_force(representative_particle_pos - particles[ip].pos) * weight;
        }
      }
    }
  }
